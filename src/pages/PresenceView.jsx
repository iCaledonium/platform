...
export default function PresenceView({ world, user, sceneData, actorName, actorPhoto, actorId: actorIdProp, encounter_id, onLeave }) {
  ...
  const lastGoodFrameTimer = useRef(null);
  ...
  function chromaKeyPixels(pixels) {
    ...
    for (let i = 0; i < data.length; i += 4) {
      ...
      if (dist < TOLERANCE) { data[i+3] = 0; }
        ...
        const a = (dist - TOLERANCE) / SOFTNESS; data[i+3] = Math.round(255 * a);
        if (KEY_G > KEY_R && KEY_G > KEY_B) { const sf = SPILL/100; data[i+1] = Math.round(g-(g-Math.max(r,b))*sf*(1-a)); }
      } else if (SPILL > 0 && KEY_G > KEY_R && KEY_G > KEY_B) {
        const sf = (SPILL/100)*Math.max(0,1-(dist-TOLERANCE-SOFTNESS)/60);
        if (sf > 0) data[i+1] = Math.round(g-(g-Math.max(r,b))*sf*0.3);
      }
    }
  }
  ...
  function stripActions(text) {
    return text
      .replace(/\nDE[LB]TAS?:[\s\S]*/gi, "")  // strip DELTAS and everything after
      .replace(/DE[LB]TAS?:[\s\S]*/gi, "")
      .replace(/\*[^*]*\*/g, "")
  ...
  function displayText(text) {
    const noTags = text
      ...
      .replace(/\s*(ACTION|LOCATION|POSITION|OUTFIT|DE[LB]TAS?):[\s\S]*/gi, "")
  ...
  function startRenderLoop() {
    ...
    function render() {
      ...
      // Capture last good frame every ~2s for fallback display
      if (!lastGoodFrameTimer.current || Date.now() - lastGoodFrameTimer.current > 2000) {
        lastGoodFrameTimer.current = Date.now();
        try { setLastGoodFrame(canvas.toDataURL("image/jpeg", 0.85)); } catch(e) {}
      }
      ...
      <div className={styles.body}>
        ...
        <div className={styles.compCol}>
          <div className={styles.wrap}>
            {videoBase && encounter_id && videoReady
              ? <canvas ref={canvasRef}
                  className={`${styles.photoCanvas} ${videoFading ? styles.photoFading : ""}`}
                  width={720} height={1280} />
              : lastGoodFrame
                ? <img src={lastGoodFrame} className={styles.photo} alt={actorName} />
                : actorPhoto
